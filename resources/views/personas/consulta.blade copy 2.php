                            ...
                            <table id="tabla" name="tabla" class="table  table-striped table-row-bordered gy-5 gs-7 ">
                                ...
                                <tbody>
                                    @php
                                        $contador = 0;
                                        $valor = '';
                                        $valor = $modeloPersonas[0]->p_persona;
                                    @endphp
                                    @foreach ($modeloPersonas as $modeloPersona)
                                        @if ($contador == 0)
                                            <tr>
                                                <td>{{ $modeloPersona->p_persona }}</td>
                                                <td>{{ $modeloPersona->juicio_id }}</td>
                                                <td>{{ $modeloPersona->internacional }} </td>
                                                <td>{{ $modeloPersona->e_descripcion }} </td>
                                                <td>{{ $modeloPersona->c_descripcion }}</td>
                                            </tr>
                                            @php
                                                $X = $modeloPersona->p_persona;
                                            @endphp
                                        @else
                                            @if ($modeloPersona->p_persona != $X)
                                                <tr>
                                                    <td>{{ $modeloPersona->p_persona }}</td>
                                                    <td>{{ $modeloPersona->juicio_id }}</td>
                                                    <td>{{ $modeloPersona->internacional }} </td>
